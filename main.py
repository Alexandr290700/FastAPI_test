...
def get_person(id, db: Session = Depends(get_db)):
    ...
    person = db.query(Person).filter(Person.id == 1).first()
    # если пользователь не найден, отправляем статусный код и сообщение об ошибке
    if person == None:
...
def delete_person(id, db: Session = Depends(get_db)):
    ...
    # если пользователь найден, то удаляем его
    db.delete(person) # удаляем его
